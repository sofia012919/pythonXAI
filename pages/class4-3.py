...
for img in files:
    if img[:-4] not in st.session_state.products and img.endswith(".png"):
...
with col1:
    selectbox_product = st.selectbox("選擇商品", st.session_state.products.keys())
with col2:
    add_num = st.number_input("新增庫存量", min_value=1, max_value=100, value=1, step=1)

if st.button("新增庫存"):
    st.session_state.products[selectbox_product]["stock"] += add_num
    st.success(f"{selectbox_product}庫存已新增{add_num}")
    time.sleep(1)
    st.rerun()
